...
Clone the git repositories for elspero and Gitian and then checkout the elspero version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the elspero repository
...
Gitian builds' in [doc/release-process.md](release-process.md) in the elspero repository.
...
For example:
```bash
URL=https://github.com/user/elspero.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/elspero-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/elspero-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/elspero-build.assert
```
...
[elspero/gitian.sigs](https://github.com/elspero-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
